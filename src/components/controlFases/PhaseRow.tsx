...
export default function PhaseRow({
  ...
  const btnClass =
    f.accionColor === "success"
      ? "bg-green-100 text-green-700 cursor-default"
      : f.accionColor === "neutral"
      ? "bg-gray-100 text-gray-600 cursor-not-allowed"
      : "bg-blue-600 text-white hover:bg-blue-700";

  const canApprove = !f.accionDisabled && f.accionColor === "primary";
  ...
  return (
    <tr className="border-t">
      ...
      <td className="px-6 py-4">
        <span className={`rounded-full px-3 py-1 text-xs ${faseClass}`}>
          {f.faseActual}
        </span>
      </td>
      <td className="px-6 py-4">
        <ProgressBar done={f.progresoHecho} total={f.progresoTotal} />
      </td>
      <td className="px-6 py-4">
        <ul className="space-y-1 text-xs">
          <li className="flex items-center gap-1 text-emerald-600">
            <LuCircle /> Clasificados: {f.resumen.clasificados}
          </li>
          <li className="flex items-center gap-1 text-amber-600">
            <LuCircle /> No clasificados: {f.resumen.noClasificados}
          </li>
          <li className="flex items-center gap-1 text-red-600">
            <LuCircle /> Descalificados: {f.resumen.descalificados}
          </li>
        </ul>
      </td>
      <td className="px-6 py-4">
      ...
      <td className="px-6 py-4">
        <span className={`rounded-full px-3 py-1 text-xs ${estadoClass}`}>
          {f.estado}
        </span>
      </td>
      <td className="px-6 py-4">
