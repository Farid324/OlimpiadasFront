...
const schema = z.object({
  nombre: z.string().min(1, 'El nombre es obligatorio')
    ...
    .refine(v => v.trim().split(/\s+/).length >= 2, { message: 'Debe ingresar nombre y apellido' })
    .refine(v => v.trim().split(/\s+/).every(w => /^[A-ZÁÉÍÓÚÑ][a-záéíóúñ]+$/.test(w)), {
      message: 'Cada nombre y apellido debe iniciar con mayúscula'
    }),
  correo: z.string().min(1, 'El correo es obligatorio')
    .email('Correo inválido')
    .refine(v => !v.includes(' '), { message: 'El correo no debe contener espacios' }),
  telefono: z.string().min(8, 'El teléfono debe tener 8 dígitos').max(8, 'El teléfono debe tener 8 dígitos')
    .regex(/^[0-9]+$/, 'Solo se permiten números'),
  ci: z.string().min(6, 'El CI debe tener entre 6 y 8 dígitos').max(8, 'El CI debe tener entre 6 y 8 dígitos')
    .regex(/^[0-9]+$/, 'El CI solo debe contener números'),
  institucion: z.string().min(2, 'La institución es obligatoria')
    .regex(/^[A-Za-zÁÉÍÓÚÑáéíóúñ\s]+$/, 'Solo se permiten letras'),
  experiencia: z.string().regex(/^[0-9]{1,2}$/, 'Debe ingresar 1 o 2 dígitos numéricos')
    .refine(v => { const n = Number(v); return n >= 1 && n <= 30; }, {
...
export default function RegisterResponsableModal({
  ...
  const [lockAfterSuccess, setLockAfterSuccess] = useState(false); // deshabilita UI durante los 3s
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (mode === 'create') {
        ...
        if (tel.data.exists) { setDupError('❌ El teléfono ya está registrado'); setLoading(false); return; }
        if (ci.data.exists) { setDupError('❌ El CI ya está registrado'); setLoading(false); return; }
        if (correo.data.exists) { setDupError('❌ El correo ya está registrado'); setLoading(false); return; }
      }
      ...
      if (mode === 'create') {
        ...
        if (areaRes.data?.exists) { setDupError('❌ Ya existe un responsable asignado a esta área'); setLoading(false); return; }
      } else if (mode === 'edit') {
      ...
      if (mode === 'edit' && initial?.id_usuario) {
        ...
        // Para edición mantenemos el flujo normal
        ...
        // Mostrar banner unos segundos y luego cerrar/avisar al padre
        ...
        setTimeout(() => {
          ...
          onClose();       // y recién cerramos el modal
        }, 1000);
      }
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-2 sm:p-3">
      <div className="bg-white p-4 sm:p-6 rounded-xl w-full max-w-lg sm:max-w-xl md:max-w-2xl relative max-h-[90vh] overflow-y-auto">
        ...
        <p className="text-gray-500 mb-4">
          {mode === 'edit' ? 'Actualice la información del responsable' : 'Complete la información del responsable'}
        </p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className={lockAfterSuccess ? 'pointer-events-none opacity-75' : ''}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                onKeyDown={(e) => { const block = ['-', '+', 'e', 'E', '.']; if (block.includes(e.key)) e.preventDefault(); }}
                onInput={(e) => { const el = e.currentTarget as HTMLInputElement; el.value = el.value.replace(/\D/g, '').slice(0, 2); }}
                onPaste={(e) => { const text = e.clipboardData.getData('text'); if (!/^\d{1,2}$/.test(text)) e.preventDefault(); }}
              />
            ...
            <div>
              ...
              <select
                {...register('id_area')}
                className={`border rounded-md p-2 w-full text-sm ${selectedArea === '0' ? 'text-gray-400' : 'text-black'}`}
              >
          ...
          <div className="flex flex-col md:flex-row justify-end gap-2 mt-6">
            <Button onClick={onClose} variant="outline" type="button" className="w-full md:w-auto" disabled={lockAfterSuccess}>
              Cancelar
            </Button>
