...
export default function RegisterTutorModal({
  onClose,
  onSuccess, // callback -> devuelve telefono (para setear tutorContacto) y resumen
}: {
  onClose: () => void;
  onSuccess: (res: {
    telefono: string;
  ...
  // Buscar tutores (debounced simple)
  useEffect(() => {
  ...
  const pickTutor = async (t: Tutor) => {
    // Carga el detalle por si quieres usar más info y el contador real
    try {
      ...
      // fallback con los datos del item
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white p-6 rounded-xl w-[720px] relative">
        <button
          onClick={onClose}
        ...
        <p className="text-gray-500 mb-3">Complete la información del tutor</p>
        ...
        <div className="relative mb-4">
          ...
          {/*Resultados rápidos*/}
          {results.length > 0 && (
            <div className="absolute z-10 mt-2 w-full bg-white border rounded-md max-h-60 overflow-y-auto">
              {results.map((r) => (
                <button
                  key={r.id}
                  onClick={() => {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input placeholder="00000000" {...register("ci")} />
              {errors.ci && (
            ...
            <div>
              ...
              <Input placeholder="+591 7xxxxxxx" {...register("telefono")} />
              {errors.telefono && (
                <p className="text-red-500 text-sm">
